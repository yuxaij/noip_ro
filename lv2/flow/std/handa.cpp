...
struct heap {
    struct node {
        int value;   // here we use the address trick to make the value accessable by *(heapnode_instance).
        void swap(node *n) { value = n->value; }
        bool operator () {
        }
    };
    heap(int maxn) {
        this->p = (int*) malloc(sizeof(int) * maxn + 1);
        this->n = 0;
    }
    bool empty() { return heap_n == 0; }
    void clear() {
        n = 0;
    }
    void insert(int v) {
    ...
    void erase(int *addr) {
        node *cur = (node*) addr;
        if (cur != nodes) {
            cerr << "heap::erase should receive the address of top node." << endl;
            exit(0);
        }
        node *leafer = nodes + (n >> 1); 
        while (cur <= leafer) {
            int lp = (cur - nodes) << 1;
            lef = nodes + lp;
            rig = nodes + lp + 1; 
            if (lef < rig) {
                nodes[pos].value = nodes[lefp].value;
                nodes[lefp].value = nodes[rigp].value;
